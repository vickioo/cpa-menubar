...
func TestSub2AccountSelectionAndSanitization(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if include != tt.include { t.Fatalf("include=%v, want %v", include, tt.include) }
			if include && account.Focus != tt.focus { t.Fatalf("focus=%v, want %v", account.Focus, tt.focus) }
		})
	}
	...
	for _, forbidden := range []string{"valid@example.com", "credentials", "access_token", "refresh_token", "password"} {
		if strings.Contains(payload, forbidden) { t.Fatalf("response leaked %q: %s", forbidden, payload) }
	}
	if account.DisplayName != "v***@example.com" { t.Fatalf("unexpected mask: %q", account.DisplayName) }
}
